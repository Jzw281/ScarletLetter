...
let main_menu = true;
let prolouge = false;
let hester_on_scaffolding = false;
let roger_cutscene = false;
let governor = false;
let governor_admit = false;
let guilt = false;
let guilt_admit = false;
let woods = false;
let woods_admit = false;
let woods_mad = false;
let election = false;
let election_escape = false;
let election_noAdmit = false;
let election_kill = false;
let escape = false;
let noEscape = false;
let killRoger = false;
let madHester = false;
...
let hester_option1 = false;
let hester_option2 = false;
let hester_option3 = false;
let governor_option1 = false;
let governor_option2 = false;
let governor_option3 = false;
let governor_admit_option1 = false;
let governor_admit_option2 = false;
let governor_admit_option3 = false;
let guilt_option1 = false;
let guilt_option2 = false;
let guilt_option3 = false;
let guilt_admit_option1 = false;
let guilt_admit_option2 = false;
let guilt_admit_option3 = false;
...
function textDisplay( Text,  x,  _y, boxWidth,  size){
  fill(0);
  textSize(size);
  
  let y = _y + 20; /////////////JOZA: I CHANGED THE OLD 'Y' to 'y' and the old 'y' to '_y' cuz it's sus
  let text_length = "";
...
function button( ChoiceOne,  ChoiceTwo,  ChoiceThree){
  //draw the button
  fill(255, 70);
  rect(10,490,250,50);
...
function drawScene ( choiceOne,  choiceTwo,  choiceThree,  scene,  nextScene1,  nextScene2, nextScene3){
...
function draw(){
  if(main_menu){
    ...
    if(mouseX>=400-textWidth("start") && mouseX<=400+textWidth("start") && mouseY>=580 && mouseY<=670){
      ...
      if(mouseIsPressed){ //if the button is clicked, go to prolouge
        textAlign(LEFT);
        prolouge = true;
        main_menu = false;
      }
  ...
  text("(press enter to continue)", 550,775);
    ...
    if (dist(mouseX, mouseY, 750, 50) < 20 && mouseIsPressed) {
      prayAmt += 10;
    }
...
function reset(){ //resets to main menu
  main_menu = true;
  prolouge = false;
  hester_on_scaffolding = false;
  roger_cutscene = false;
